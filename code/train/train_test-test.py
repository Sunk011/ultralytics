...
if __name__ == '__main__':
    ...
    results = model.train(
        ...
        # ======================================================================
        data='/home/sk/datasets/ir_voc/ir_voc.yaml',   # 数据集配置文件的路径。
        epochs=5,                     # 训练的总轮次 (Total number of training epochs)。
        batch=16,                       # 每批次的图像数量 (Batch size)。-1会自动根据GPU显存调整。
        imgsz=640,                      # 输入图像的尺寸 (Image size)。可以是整数640，也可以是矩形[640, 480]。
        ...
        # # ======================================================================
        # optimizer='auto',               # 选择优化器 ('SGD', 'Adam', 'AdamW', 'auto')。'auto'会自动选择最佳的。
        ...
        # mixup=0.0,                      # MixUp数据增强的概率 (MixUp augmentation): 将两张图片混合。
        ...
        # # ======================================================================
        project='usage/tmp',           # 训练结果保存的项目目录 (Project directory)。
        # name='11m-1280-b64-e180',                     # 本次训练的名称 (Experiment name)。
        ...
        # # ======================================================================
        device='7',                    # 指定运行设备 ('cpu', '0', '0,1,2,3', or None)。None会自动选择可用GPU。
        ...
        # patience=50,                    # 早停耐心值 (Early Stopping Patience): 连续N个epoch性能未提升则提前停止训练。
