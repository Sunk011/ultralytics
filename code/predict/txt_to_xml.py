...
"""
从检测结果文件转换为XML格式
支持从txt文件读取检测结果并保存为PASCAL VOC XML格式
"""
...
class TxtToXMLConverter:
    ...
    def parse_detection_txt(self, txt_path: str) -> List[Tuple[int, float, List[float]]]:
        """
        解析检测结果txt文件
    ...
    def print_detection_stats(self, txt_path: str) -> None:
        ...
        for class_name, count in sorted(class_counts.items()):
            ...
            print(f"{class_name:12}: {count:3d} 个目标 | "
                  f"置信度 - 平均: {avg_conf:.3f}, 最小: {min_conf:.3f}, 最大: {max_conf:.3f}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='将检测结果txt文件转换为PASCAL VOC XML格式')
    parser.add_argument('--txt_file', '-t', type=str, default='5-04.txt',
                       help='检测结果txt文件路径')
    parser.add_argument('--image_path', '-i', type=str, default='5.jpg',
                       help='对应的图像文件路径')
    parser.add_argument('--output', '-o', type=str, default=None,
                       help='输出XML文件路径（默认根据txt文件名生成）')
